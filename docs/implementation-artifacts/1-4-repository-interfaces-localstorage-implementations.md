...
2. **Given** the localStorage implementations, **When** data is written, **Then** it lands under `gol:battles`/`gol:organisms`/`gol:settings` with dense `gridState` at rest, and `gol:schema` is stamped on the very first workspace-data write (AR-9/11) — a settings-only write deliberately does not stamp it (Decision F; resolved in review, see Review Findings)
...
_Code review 2026-08-05 — three layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor). 2 decision-needed (both resolved by Sidiar), 10 patch, 2 deferred, 7 dismissed as noise._

_The implementation is unusually self-documenting — the Dev Agent Record actively flags its own interpretation gaps (the settings/stamp decision below) rather than hiding them, and the quota/corruption/clearAll invariants are covered by tests that assert the actual guarantee, not just the happy path. The findings below are mostly about fault isolation at the collection level and a few sharp edges the story's own traps didn't name._

- [x] [Review][Patch] **Settings-only writes never stamp `gol:schema`, narrowing AC2's literal text** — AC2 says the stamp lands "on the very first write"; `LocalStorageSettingsRepository.save()` calls `writeSettingsKey`, which deliberately skips `stampSchemaVersion()` (Decision F: settings are device-local, not workspace data, and stamping on a theme toggle would leave a stamped-but-empty store that Story 1.5 reads as "already initialized," silently skipping the Conway's Classic seed). The Dev Agent Record self-flags this as "a fourth decision... NOT in the story — please review it." The reasoning is sound; the AC text was never updated to match. **Resolved (Sidiar): accept the shipped behavior — reword AC2** from "on the very first write" to "on the very first workspace-data write" so the AC matches the (correct) code. [packages/persistence/src/storage.ts:135-137, packages/persistence/src/localStorageSettingsRepository.ts:31-33, story AC2]
- [x] [Review][Patch] **`list()`/`listFull()` abort entirely on the first corrupt record, with no partial/fault-tolerant path** — one bad record among up to ~50 makes `BattleRepository.list()` (and `OrganismRepository.list()`) throw instead of returning the other N-1 good summaries, taking down the whole Gallery listing rather than just the one corrupt entry. `listFull()` has the same shape: it discards every already-loaded battle in the loop the moment one `load(id)` throws. **Resolved (Sidiar): fix now** — `list()` (both repositories) skips a record that fails its projection schema and continues, returning the good summaries; `listFull()` likewise skips and continues rather than aborting mid-loop. [packages/persistence/src/localStorageBattleRepository.ts:45-68, packages/persistence/src/localStorageOrganismRepository.ts:36-48]
- [x] [Review][Patch] `stampSchemaVersion()`'s own quota failure isn't caught inside `writeDataKey()` — if the data write just barely fits but the trailing stamp write doesn't, `QuotaExceededError` propagates to the caller even though their save genuinely succeeded; the Dev Notes call "data written, stamp write fails" the *recoverable* failure mode, but nothing here actually treats it that way from the caller's perspective [packages/persistence/src/storage.ts:124-127]
- [x] [Review][Patch] `apps/web/lib/repositoryFactory.ts` imports mode via relative `./mode` instead of the `@/lib/mode` alias both the story (Task 8) and `project-context.md` specify verbatim — functionally identical today since both files sit in `apps/web/lib/`, but it's a citable deviation from a rule quoted in two authoritative docs, and this file is the first to establish the convention either way [apps/web/lib/repositoryFactory.ts:3]
- [x] [Review][Patch] Reserved-key collision (`__proto__`) on organism `id` silently corrupts the in-memory collection — `collection[id] = record` is a plain-object assignment with no guard, and `OrganismSchema.id` is a bare `z.string().min(1)` (deliberately, to allow `'conways-classic'`), so nothing upstream blocks an id of `'__proto__'`. `BattleSchema.id` is `z.uuid()`, which already blocks this structurally, so only the organism repository needed the guard. Added `assertSafeCollectionId()` in `storage.ts`, called from `save()`/`replaceAll()` [packages/persistence/src/storage.ts, packages/persistence/src/localStorageOrganismRepository.ts]
- [x] [Review][Patch] `SettingsSchema`'s `defaultGridSize` uses a shared, non-frozen default object (`.default({cols:100,rows:60})` instead of a factory `.default(() => ({...}))`), and `DEFAULT_SETTINGS` itself isn't frozen — every parse that falls back to the default shares one object reference, so an in-place mutation by a future consumer would corrupt the shared singleton and every other Settings instance that used the default [packages/domain/src/settingsSchema.ts:24,38]
- [x] [Review][Patch] `repositoryFactory.test.ts`'s `Storage.prototype` spies aren't restored in `afterEach`, unlike the other three new test files (`storage.test.ts`, `localStorageBattleRepository.test.ts`) which all call `vi.restoreAllMocks()` — harmless today, inconsistent mock hygiene for the next person who extends this file [apps/web/lib/repositoryFactory.test.ts:15-18]
- [x] [Review][Patch] The `6_000_000`-byte magic number in the real-jsdom-quota test has no inline comment explaining why that size reliably exceeds jsdom's default quota — if jsdom's default ever changes upstream, the test silently stops testing what it claims to, and nobody reading the test in isolation would know why [packages/persistence/src/storage.test.ts]
- [x] [Review][Patch] `exists()` reports `true` for a present-but-corrupt record that `load()`/`list()` would reject — a real and reasonable asymmetry (existence vs. validity are different questions) but undocumented and untested; worth a one-line JSDoc note on the interface [packages/persistence/src/repositories.ts:25,42]
- [x] [Review][Patch] `readStoredValue`/`SettingsRepository.load()` treats a literal JSON `null` at `gol:settings` as "absent" (via `stored ?? {}`), inconsistent with `readCollection()`'s explicit `null` → `CorruptDataError` handling for battles/organisms — not reachable through any current write path, but an inconsistent defensive posture across the same module [packages/persistence/src/storage.ts:86-94, packages/persistence/src/localStorageSettingsRepository.ts:17-21]
- [x] [Review][Defer] `replaceAll()` silently drops earlier entries when given duplicate ids in the input array [packages/persistence/src/localStorageBattleRepository.ts:81-85, packages/persistence/src/localStorageOrganismRepository.ts:65-69] — deferred, pre-existing scope boundary; belongs to Story 5.8's atomic-import validation, matching this story's explicit "referential-integrity is the caller's job" stance for `delete()`
- [x] [Review][Defer] ~~`listFull()` re-reads and re-`JSON.parse`s the whole `gol:battles` collection once per battle instead of reusing the already-read id list's collection~~ [packages/persistence/src/localStorageBattleRepository.ts:59-68] — **resolved incidentally**: the patch above rewrote `listFull()` to iterate the already-read collection directly instead of calling `load(id)` per id, which also eliminated the redundant re-parses

**Dismissed as noise (7):** `save()` methods don't re-`parse()` their `Battle`/`Organism`/`Settings` argument before persisting (correct per project-context's "Zod parses at boundaries, not everywhere" — callers already hold a type-checked value) · `readCollection` typed `Record<string, unknown>` rather than the Dev Notes' suggested `z.input<typeof BattleSchema>` (a sound alternative, fully covered by round-trip tests, arguably reinforces "parse at boundaries" more strongly) · mismatched RFC citation for `clearAll`'s placement rationale between the story's task-3 code sketch ("RFC-001") and the shipped code comment ("RFC-006 Alternative 1") — a documentation-only inconsistency, not a functional issue · no sort/order guarantee on `list()` (by design — `BattleSummary.updatedAt` exists precisely so the future Gallery sorts client-side) · `repositoryFactory.ts`'s plain `throw new Error(...)` for an unsupported `NEXT_PUBLIC_MODE` (a fail-fast build-misconfiguration guard, not a runtime error callers are meant to discriminate, unlike `QuotaExceededError`/`CorruptDataError`) · `writeSettingsKey`'s quota-failure path being "untested" (it delegates to the same `writeKey()` already exercised by `writeDataKey`'s test suite — a duplicate test would be coverage-padding on identical code) · `clearAll()`'s test matrix missing a "only one of two data keys populated" case (`removeItem` on an absent key is a universal no-op).
...
Modified (2026-08-05 code review — see Review Findings):

- packages/persistence/src/storage.ts (`writeDataKey` swallows a stamp-only quota failure; new `assertSafeCollectionId`)
- packages/persistence/src/storage.test.ts (stamp-quota-swallow test; magic-number comment)
- packages/persistence/src/localStorageBattleRepository.ts (`list()`/`listFull()` skip-and-continue on a corrupt record)
- packages/persistence/src/localStorageBattleRepository.test.ts
- packages/persistence/src/localStorageOrganismRepository.ts (`list()` skip-and-continue; `assertSafeCollectionId` guard on `save()`/`replaceAll()`)
- packages/persistence/src/localStorageOrganismRepository.test.ts
- packages/persistence/src/localStorageSettingsRepository.ts (literal `null` at rest now treated as corrupt, not absent)
- packages/persistence/src/localStorageSettingsRepository.test.ts
- packages/persistence/src/repositories.ts (`exists()` doc note)
- packages/domain/src/settingsSchema.ts (`defaultGridSize` factory default; `DEFAULT_SETTINGS` frozen)
- apps/web/lib/repositoryFactory.ts (`@/lib/mode` alias import)
- apps/web/lib/repositoryFactory.test.ts (`vi.restoreAllMocks()` in `afterEach`)
- apps/web/vitest.config.mts (new `resolve.alias` for `@/*` — required to make the `@/lib/mode` import resolve under Vitest, which unlike Next's own build doesn't read tsconfig `paths` on its own)
- docs/implementation-artifacts/deferred-work.md (2 new deferrals; 1 pre-existing deferral resolved incidentally)
...
- 2026-08-05: Code review (3 layers) — 2 decisions resolved by Sidiar (AC2 reworded to "very first workspace-data write" to match the shipped settings/stamp split; `list()`/`listFull()` rewritten to skip a corrupt record and keep going instead of aborting the whole listing — this also incidentally fixed a deferred perf item by removing `listFull()`'s per-id collection re-reads). 10 patches applied: a stamp-write quota failure no longer surfaces as a false "save failed"; `repositoryFactory.ts` now imports via the `@/lib/mode` alias (required adding a matching `resolve.alias` to `apps/web/vitest.config.mts`, which had none); a reserved-id guard (`assertSafeCollectionId`) blocks `'__proto__'` from silently vanishing out of the organism collection; `SettingsSchema.defaultGridSize` now uses a factory default and `DEFAULT_SETTINGS` is frozen, closing a shared-mutable-reference risk; plus test-hygiene and documentation fixes (mock cleanup, a magic-number comment, an `exists()` doc note, and settings' literal-`null` handling aligned with `readCollection()`'s). 2 items deferred to Story 5.8 (duplicate-id handling in `replaceAll()`) and noted as pre-existing scope. 7 dismissed as noise. 6 new tests (149 total, was 143); persistence coverage 99.13% stmts / 95.74% branch. Verified: `npm run ci` exit 0 (typecheck, lint, format, coverage, build:standalone, bundle 180.9KB/300KB, e2e 8/8). Status → done.
