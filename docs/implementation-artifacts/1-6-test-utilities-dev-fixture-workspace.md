...
_Code review 2026-08-05 (three parallel layers: blind adversarial, edge-case, acceptance audit). 17 patch items, 2 deferred, 5 dismissed as noise. No decision-needed items._

_The load-bearing parts of this story held up under independent scrutiny: all 8 mock-rule `contentHash` literals reproduce byte-for-byte under the pinned canonicalization (and all 10 ids/hashes are unique across the mock set plus Conway's two); `organismIds` ≡ the placed set parses clean under `superRefine` for both battles; the `=== 'development'` guard, its placement inside the effect, the `isFreshWorkspace()` pre-read ordering, and the dynamic import are each correct and correctly justified; the `hasRun`/`mounted` ref pair is untouched; and the production DCE was independently re-verified (`grep -rl` over `apps/web/out` for seven fixture strings returns zero files). The AR-45 coverage matrix is genuinely derived from the fixtures at run time, not hand-listed. The two findings that matter are both cases where a ticked checkbox does not correspond to shipped code._

- [x] [Review][Patch] **Conway's Classic is never re-exported from `@gol/test-utils` — AC1's canonical set is incomplete** — AC1 requires the package to provide "the canonical organisms (Conway's Classic + the three PRD organisms)", and Task 3/Task 6 both spell out the re-export explicitly. `index.ts` exports no Conway symbol, and `mockWorkspace.ts` imports only `CONWAYS_CLASSIC_ID`. `createMockWorkspace()` therefore returns a 3-organism set with no baseline, and every downstream consumer (3.x, 4.19–4.21, 5.3–5.8) has to reach into `@gol/domain` separately. The Dev Agent Record records the inverse fix — "`mockWorkspace.ts` imported `CONWAYS_CLASSIC` without using it; removed the unused import" — the lint error was the signal that the required re-export was missing, and it was resolved by deleting the import. [packages/test-utils/src/index.ts:10-16]
- [x] [Review][Patch] **Mock survival rules are shared, unfrozen module singletons — the factories are shallow** — Task 3 requires "each call returns fresh objects", justified by "these are test INPUTS that tests will mutate". Only the outer organism object and the `survivalRules` array are fresh; every `SurvivalRule`, its `conditions` array and its `payload` are module-level consts shared process-wide and unfrozen. Verified by execution: mutating `createMockOrganisms()[0].survivalRules[0].payload.action` changes what a *subsequent* call returns. Cross-test contamination inside a Vitest worker, and the stated protection does not exist. (Battles are genuinely fresh per call.) [packages/test-utils/src/mockWorkspace.ts:41-138,147-182]
- [x] [Review][Patch] **The factory-identity test is structurally incapable of catching the above** — it asserts `first !== second` and `first[0] !== second[0]` only; `first[0].survivalRules[0]` is never compared, so it passes unchanged with 100% shared nested state. [packages/test-utils/src/mockWorkspace.test.ts:103-116]
- [x] [Review][Patch] **`Rng` is published as an exported package type, against an explicit scope boundary** — "❌ The `Rng` interface as a published type — `@gol/simulation`, Epic 3. Structural shape only, here." Task 5 specifies the inline return type `{ int(maxExclusive: number): number }`. The local declaration is fine and the `@gol/simulation` dependency was correctly avoided, but `export type { Rng }` from the index is the published type the boundary bans, and it sets up a name collision when Epic 3 declares the real one. [packages/test-utils/src/index.ts:21]
- [x] [Review][Patch] **The non-`raw` `FakeSeed` path performs no runtime validation** — Task 2: "the `raw` escape hatch writes records bypassing validation … everything else validates on the way in", and the file's own doc comment repeats the claim. Both paths do a bare `roundTrip` with no `Schema.parse`, so only the compile-time annotation applies and a seed cast `as Battle` behaves exactly like `raw` with no signal. [packages/test-utils/src/fakeRepositories.ts:66-74]
- [x] [Review][Patch] **`settings.load()` hands back the shared frozen `DEFAULT_SETTINGS`; the real repository returns a fresh parse** — `LocalStorageSettingsRepository.load()` does `SettingsSchema.safeParse(stored === undefined ? {} : stored)`, a new object each call. `DEFAULT_SETTINGS` is `Object.freeze(SettingsSchema.parse({}))`, so a test that mutates loaded settings passes against localStorage and throws against the fake — the inverse of the reference-vs-copy trap `roundTrip()` is careful about everywhere else in the file. [packages/test-utils/src/fakeRepositories.ts:194]
- [x] [Review][Patch] **Fake organism `save()`/`replaceAll()` omit the `assertSafeCollectionId('__proto__')` guard the real repository enforces** — it is the first statement in both real methods. A test asserting the rejection path sees a successful save against the fake. No shipped fixture is affected (the `mock-*` ids are safe). [packages/test-utils/src/fakeRepositories.ts:148,184]
- [x] [Review][Patch] **An empty `raw` map stamps the workspace while an empty validated seed does not** — `if (seed?.raw?.battles) { …; stamped = true; }` is unconditional, so `createFakeRepositories({ raw: { battles: {} } })` reports `isFreshWorkspace() === false` with zero records. The validated path correctly guards on `length > 0`. Given how load-bearing the stamp is, the two paths must agree. [packages/test-utils/src/fakeRepositories.ts:75-82]
- [x] [Review][Patch] **`placePattern` accepts ragged patterns and ragged base grids** — `patternCols`/`cols` are read from index `0` only. A short pattern row writes `undefined` into a `number[][]`; a long one is silently clipped past the bounds check; a ragged base grid writes past a shorter row's end, creating holes that serialize as `null`. `gridFromPattern` two doc-blocks up commits to the opposite policy — throw eagerly, because deferring "produces a `BattleSchema` superRefine failure hundreds of lines away from its cause". No test covers a ragged pattern. [packages/test-utils/src/gridBuilders.ts:61-79]
- [x] [Review][Patch] **`placePattern` accepts non-integer and `NaN` offsets** — `placePattern(grid, [[1]], 0.5, 0)` passes the bounds check and writes to property `"0.5"`, which vanishes on `JSON.stringify`; a `NaN` offset makes every bounds comparison false and yields an opaque `TypeError` instead of the intended spills-outside error. [packages/test-utils/src/gridBuilders.ts:66]
- [x] [Review][Patch] **`emptyGrid` silently coerces negative, fractional and `NaN` dimensions** — `Array.from({ length: -5 })` yields `[]`, so `emptyGrid(-5, 3)` returns three empty rows and the failure surfaces as a `gridSize` refine far from its cause. Dimensions being parameters (Decision A) is exactly why they deserve a guard. [packages/test-utils/src/gridBuilders.ts:13-15]
- [x] [Review][Patch] **`gridFromPattern`'s ragged check counts UTF-16 units while its cells count code points** — width is `rows[0].length` but cells come from `Array.from(row, …)`. `gridFromPattern(['A😀','AAA'], {A:1,'😀':2})` passes the ragged guard (both `.length === 3`) and returns `[[1,2],[1,1,1]]` — a ragged grid out of the function whose stated job is rejecting ragged input. [packages/test-utils/src/gridBuilders.ts:28-33]
- [x] [Review][Patch] **`gridFromPattern` does not range-check legend values** — a legend mapping a character to a negative, fractional or out-of-range value defers to a `BattleSchema` int/min/max failure, which is the deferral this function exists to prevent. [packages/test-utils/src/gridBuilders.ts:36-43]
- [x] [Review][Patch] **`int()` violates its own documented contract: `int(0)` returns `NaN`, and plain modulo is not uniform** — the JSDoc promises "a uniform integer in `[0, maxExclusive)`" and Task 5 says the same; the implementation is `nextUint32() % maxExclusive`. `int(0)` yields `NaN` (a caller then indexes with `NaN` and gets `undefined`, not an error), `int(-5)` returns a negative, `int(2.5)` returns a non-integer, and modulo over a 32-bit range is biased for any non-power-of-two bound. Low practical impact for the FR-5.4 tie-break, but Epic 3 consumes this as the seeded generator. Also: `seed >>> 0` silently aliases `1.5 → 1` and `NaN → 0`. [packages/test-utils/src/seededRng.ts:12-14,26,38-40]
- [x] [Review][Patch] **`placeMockRoster` places an absent roster id as empty cells** — `organismIds.indexOf(id) + 1` yields `0` (the empty-cell value) when the id is missing, so the organism is silently not placed rather than throwing. Currently unreachable — both call sites pass correct rosters, and `BattleSchema.superRefine` plus the parse test would catch the resulting zero-placed-cells battle — but the failure message would point at Decision H.1, not at the roster mismatch. [packages/test-utils/src/mockWorkspace.ts:196-198,230]
- [x] [Review][Patch] **Comment and record inaccuracies** — this repo treats comments as load-bearing, and five are wrong: (a) "A small 3x3 block per organism" above three 2×2 patterns; (b) "organismIds order here MUST match the placement order below" when `indexOf` makes order irrelevant — the comment describes a constraint the code deliberately does not have; (c) the e2e comment claims the assertions prove the fixtures are "truly unreachable end-to-end", but `localStorage` contents hold equally if the module ships bundled and merely uninvoked — the actual DCE evidence is the `out/` grep, which is manual and lives only in the Dev Agent Record; (d) `seedDevFixtures.test.ts`'s "this is the test that would catch Battle B referencing Conway when Conway has not been seeded" sits above a test that pre-seeds Conway, so it can never catch that case; (e) the Dev Agent Record's "byte-identical to the Story 1.5 baseline" is unsupportable — production-retained code was added to `useWorkspaceSeed.ts` (the `isFreshWorkspace()` call and two `.then` links survive DCE), so the bundle is equal to one decimal place, not byte-identical. [packages/test-utils/src/mockWorkspace.ts:194-200; apps/web/e2e/home.spec.ts:35-40; packages/test-utils/src/seedDevFixtures.test.ts:35-36; this file, Debug Log References]
- [x] [Review][Patch] **Test-assertion gaps** — the `seedDevFixtures` referential-integrity loop has no length assertion, so it passes vacuously if seeding writes zero battles (the exact break it guards); the `createMockBattles()` determinism case omits the `toEqual(second)` its organism counterpart has, leaving the fixed timestamps and generated `gridState` — the values where a regression is actually plausible — unchecked; the timestamp case asserts "older than one day" rather than the fixed literals Task 3 requires, so it would pass against `new Date(Date.now() - 2 days)`; and `seededRng.test.ts` never probes `int(0)` or a non-integer seed. [packages/test-utils/src/seedDevFixtures.test.ts:45-51; packages/test-utils/src/mockWorkspace.test.ts:110-116,170-179; packages/test-utils/src/seededRng.test.ts:19-40]
- [x] [Review][Defer] **The AR-45 coverage-matrix *universe* is hand-listed rather than derived from the schema** [packages/test-utils/src/mockWorkspace.test.ts:14-21] — deferred. The *covered* sets are correctly derived from the fixtures at run time, which is what Task 6 asked for; but `ALL_CONDITION_PROPERTIES`/`ALL_OPERATORS` are local literals, so adding a seventh operator upstream would leave the test green and the "full coverage" claim false. `survivalRuleSchema.ts` does not export the enums as constants and the properties span a discriminated union, so deriving them needs a small domain-side change.
- [x] [Review][Defer] **`seedDevFixtures` partial failure leaves a half-seeded, already-stamped workspace that never retries** [packages/test-utils/src/seedDevFixtures.ts:17-24] — deferred. A rejection mid-loop (quota on Battle B's 100×60 grid) leaves some fixtures written; the stamp is already set, so `fresh` is false on the next load and the seed never completes or retries. The story explicitly put quota-failure handling out of scope ("❌ No quota-failure injection … add them in the story that first needs one").

**All 17 patches applied 2026-08-05.** Summary of what changed:

- `packages/test-utils/src/index.ts` — re-exports `CONWAYS_CLASSIC` / `CONWAYS_CLASSIC_ID` as a pass-through of the single `@gol/domain` definition (AC1's canonical set is now complete); the `Rng` type export is gone and `createSeededRng`'s return type is structural and inline.
- `packages/test-utils/src/mockWorkspace.ts` — a `cloneRule()` JSON round-trip makes the factories deeply fresh; `placeMockRoster` looks roster indices up through a `cellValueFor()` helper that throws on a miss instead of silently yielding the empty-cell value; the "3x3 block" and "order MUST match" comments now describe what the code does.
- `packages/test-utils/src/fakeRepositories.ts` — non-`raw` seeds parse through their schema (the doc comment's promise is now true); `settings.load()` returns a fresh `SettingsSchema.parse({})` rather than the frozen `DEFAULT_SETTINGS` singleton; organism `save`/`replaceAll` enforce `assertSafeCollectionId`, newly exported from `@gol/persistence` so both stores share one definition rather than duplicating the rule; an empty `raw` collection no longer stamps the workspace.
- `packages/test-utils/src/gridBuilders.ts` — `emptyGrid` rejects non-integer/negative dimensions; `gridFromPattern` measures width in code points and range-checks legend values against the 0–255 cell encoding; `placePattern` rejects ragged patterns, ragged base grids, and non-integer/`NaN` offsets.
- `packages/test-utils/src/seededRng.ts` — `int()` throws on a non-positive-integer bound and uses rejection sampling so the documented uniformity is real; a non-integer seed is rejected rather than coerced.
- Tests — 20 new cases (75 in `@gol/test-utils`, up from 55): deep-freshness and mutation-leak coverage for the factories, literal-pinned fixture timestamps, `createMockBattles()` determinism, the four fake-vs-real contract guards, the new builder/RNG rejections, RNG uniformity, and a `seedDevFixtures` case pinning the dangling-Conway ordering requirement. The referential-integrity loop got the length assertions that stop it passing vacuously.
- Comments/record — the e2e comment now distinguishes "never seeded in production" (what it proves) from "eliminated from the bundle" (what the `out/` grep proves); the Dev Agent Record's "byte-identical" bundle claim is corrected.

**Re-verified after the patches:** `npm run ci` green end-to-end — typecheck 5/5, lint clean, `format:check` clean, **253 tests** pass (was 233), bundle **246.7 KB gzip / 300 KB** (unchanged), e2e **12/12** across chromium/firefox/webkit/tablet. DCE re-checked directly: `grep -rl` over `apps/web/out` for seven fixture strings returns **0 files** for every one.

**Dismissed as noise (5):** the `isFreshWorkspace()` pre-read "adds a new production failure mode" — false, `seedDefaultWorkspace()` already calls it internally, so the pre-read is redundant but adds no dependency; the overlapping `survive gte 2` / `die gt 5` fixture rules "resolved by array order" — false, death precedes survival by phase (M10), not by array position; a dev-fixture failure surfacing as `workspace: error` — spec-mandated, Task 4 fixes the status at three values and defines `'ready'` as both seeds finished; `list()` ordering divergence between the fake's `Map` and the real store's integer-key-hoisting plain object — no ordering contract exists and it needs numeric-string organism ids; and "an existing dev workspace never receives the fixtures" — deliberate, the `fresh` gate is the M9 self-heal protection the story mandates (worth knowing as a dev-workflow note: fixtures require clearing `localStorage`, they will not appear in an already-seeded dev profile).
...
- `npm run ci` (typecheck → lint → format:check → test:coverage → build:standalone → bundle:check → e2e) — **green end-to-end**, single run, no retries needed.
  - Bundle: **246.7 KB gzip / 300 KB budget (53.3 KB headroom)** — unchanged from the Story 1.5 baseline noted in the story brief *to the reported precision*, confirming the dev fixtures did not leak into the production bundle. (Corrected in review 2026-08-05: this originally read "byte-identical", which the diff does not support — the `isFreshWorkspace()` call and two `.then` links added to `useWorkspaceSeed.ts` survive DCE, so only the fixture branch is eliminated. The unchanged figure is evidence the *fixtures* did not ship, not that the bundle is unchanged byte-for-byte.)
...
- 2026-08-05: Code review (three parallel layers). 17 patches applied, 2 items deferred to `deferred-work.md`, 5 dismissed. Two ticked-but-unshipped subtasks found and fixed: Conway's Classic was never re-exported (AC1), and the `createMock*()` factories were shallow — every `SurvivalRule` was a shared unfrozen module singleton. `npm run ci` green after the patches (253 tests, bundle unchanged, e2e 12/12). Status → done.
