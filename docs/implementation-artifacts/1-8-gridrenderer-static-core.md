...
- [x] **Task 5: `GridRenderer`** (AC: 1, 2, 3, 4)
  ...
  - [x] **`resize(size)`** — recompute the backing store from the current CSS box, recompute the layout, invalidate the grid-line overlay cache, and full-repaint the last grid drawn. It serves both grid-dimension changes (Decision A) and canvas-box changes (fullscreen re-layout, Story 3.18) per the contract comment. It must be safe to call before any grid has been drawn (re-layout only). ⚠️ **When the new `size` is a genuine grid-dimension change, the retained grid no longer matches: skip the repaint _and drop the reference_** (amended in code review 2026-08-06). Keeping it makes the next `setGridLines()` — which repaints the last grid unconditionally — throw a dimension mismatch out of a UI event handler.
...
- [x] **Task 6: Tests** (AC: 3, 5)
  ...
  - [x] Where a fixture is needed, build it from `@gol/test-utils` (`emptyGrid`, `gridFromPattern`, `placePattern`, `createMockOrganisms`) through `toRenderableGrid`. Do not hand-roll grid literals — that is what AR-5 exists for. Import with a **relative `./` path**, matching every existing file in `apps/web/lib` (`displayColor.test.ts`, `paletteRegistry.test.ts`, `colorMath.test.ts`); `project-context.md` mandates the package name only for *cross-package* imports, and the `@/lib/...` alias this subtask originally prescribed is not the convention in this directory (corrected in code review 2026-08-06).
...
_Code review 2026-08-06 (three parallel layers: blind adversarial, edge-case, acceptance audit). 3 decision-needed, 18 patch items, 6 deferred, 6 dismissed as noise. **All decision and patch items resolved and applied the same day.** The 7 deferred items are logged in `deferred-work.md` with a revisit story each. Full `npm run ci` green after the fixes: 177 web tests (up from 157), 12/12 e2e across chromium/firefox/webkit/tablet, bundle unchanged at 246.7 KB gzip / 300 KB budget, `apps/web` coverage 95.01% stmts / 89.2% branch (up from 94.82/85.13)._

_**The two runtime fixes were verified by mutation, not just by a green suite:** re-introducing the hoisted-`beginPath` defect fails `emits exactly one beginPath, one fill, and one fillStyle write per group`, and deleting `canvasHeight / rows` from the auto-fit `Math.min` fails three tests including the fast-check property. Before the fixes, both mutations left the suite entirely green._

_**All five ACs are met and the shipped implementation is substantially correct.** The traps this story was written around landed right: `beginPath()` is inside the group loop, batching keys on `(tokenIndex, ageShade)` and never on organism, `ageShadeFor` is reused rather than re-derived (non-aging → shade 7), `max(1, …)` clamps the cell size, the background fills the whole backing store, the context is never `scale(dpr, dpr)`d, and the AC3 no-scheduling promise is real — the source genuinely contains none of the four banned primitives, verified both behaviourally and structurally. The scope sweep is clean: no `draw()`/`markDirty()`, no React, no `--gol-*`, no new dependency, no bench, no pixel snapshot. The Dev Agent Record's per-file test counts (19/5/6/10/8, 157 total) were re-counted and are **exact**, and the unchanged-bundle claim checks out — nothing under `app/` or `components/` imports any of the 11 new modules._

_**The defects cluster in two places: two real runtime bugs in `GridRenderer`'s state management, and a set of test assertions that cannot fail.** The story's own "Previous story intelligence" predicted the second cluster precisely ("the `beginPath`-count assertions (easy to write as a colour check that passes either way)") — and that is what happened: the implementation is correct, but no renderer test ever draws more than one fill group, so the assertions guarding the story's headline invariant would pass unchanged if the batching loop were broken._

- [x] [Review][Decision] **Module-global warn-once `Set`s are never reset — wrong key, cross-battle suppression, and order-dependent tests** — `warnedOutOfRangeRefs` (keyed on a bare ref number) and `warnedMissingOrganismIds` are module singletons with no clear. In a client-only SPA with no reload between battles, Battle A's corrupt `ref 3` warns once and Battle B's *genuinely different* corrupt `ref 3` is then silently skipped for the process lifetime — ref numbers are battle-relative, so the key is wrong. Both "warns exactly once" tests also pass only by being first in their file to touch that key; `vi.restoreAllMocks()` does not clear a module `Set`. Same shape as the `warnedUnknownTokens` unbounded-`Set` item already deferred from Story 1.7. Options: (a) key on something battle-scoped, (b) export a reset called per battle load and in `afterEach`, (c) scope the dedupe per-LUT/per-call, (d) extend the Story 1.7 deferral and accept it. **→ Resolved (Sidiar, 2026-08-06): (b) — export a reset and call it in the test `afterEach` blocks, killing the order-dependence now; the battle-scoped re-key is deferred to Story 1.11, the first story that loads battles into a renderer.** [apps/web/lib/colourStateGroups.ts:18, apps/web/lib/refToFillGroup.ts:25]
- [x] [Review][Decision] **Task 6 requires `@/lib/...` imports; all six new files use relative `./` — and the repo convention is `./`** — the ticked subtask says "Import via `@/lib/...` (the `vitest.config.mts` alias), not relative paths." Every new file uses `./`. Mitigating: this **matches** the established convention (`displayColor.test.ts`, `paletteRegistry.test.ts`, `colorMath.test.ts` all use `./`), and `project-context.md` mandates the package name only for *cross-package* imports. The subtask looks like the outlier, but it is ticked as done and was not done, and the conflict was not surfaced. Options: (a) amend the story subtask to match the repo convention, (b) convert the six files to `@/lib/...`, (c) accept the divergence and note it. **→ Resolved (Sidiar, 2026-08-06): (a) — the repo convention is `./` and `project-context.md` mandates the package name only for cross-package imports; the subtask text is corrected so the story stops prescribing something the codebase does not do. No source change.** [apps/web/lib/gridRenderer.ts:20-24, apps/web/lib/gridRenderer.test.ts:5-12]
- [x] [Review][Decision] **`resize()`'s skip-repaint widens a ticked subtask and is untested** — Task 5 says `resize(size)` should "full-repaint the last grid drawn", unconditionally. The implementation adds a guard that skips the repaint when the new `size` no longer matches `lastGrid`. The Dev Agent Record discloses this as "one additional design call" and admits no test exercises it. The behaviour is defensible, but it is the direct cause of the `setGridLines` crash below, so the semantics need an explicit call rather than absorption. Options: (a) accept the skip and null `lastGrid` with it, (b) repaint-on-dimension-change is the caller's job and `resize` should also drop the stale grid, (c) revert to the subtask's unconditional wording. **→ Resolved (Sidiar, 2026-08-06): (a) — keep the documented skip but drop the stale grid reference with it. This removes the `setGridLines` crash at its root rather than guarding the symptom, and releases the pinned typed arrays. Task 5's subtask text is amended to match.** [apps/web/lib/gridRenderer.ts:272-284]
- [x] [Review][Patch] **`setGridLines()` throws after a dimension-changing `resize()` — a real crash path out of a UI toggle** — `resize()` reassigns `this.size` and correctly skips the repaint when `lastGrid` no longer matches, but leaves `this.lastGrid` pointing at the stale, mismatched grid. `setGridLines()` has no such guard and repaints it unconditionally through `paint()` → `assertGridMatchesSize()` → `GridRendererDimensionMismatchError`. Sequence: `drawFull(grid100x60)` → `resize({cols:50,rows:30})` → user flips the FR-8.7 grid-lines toggle → throw. Found independently by two layers; no test covers resize-then-toggle. [apps/web/lib/gridRenderer.ts:277-284, apps/web/lib/gridRenderer.ts:288-294]
- [x] [Review][Patch] **`applyDevicePixelSizing()` compounds DPR on every call when the CSS box is 0** — `const cssWidth = this.canvas.clientWidth || this.canvas.width` mixes units: `canvas.width` is already a *backing-store* value multiplied by `dpr`, but the fallback feeds it back through `* dpr`. `clientWidth` is 0 for a detached, pre-layout, or `display:none` canvas — exactly the states the doc comment says the fallback exists for. At DPR 2 the backing store goes 2× → 4× → 8× across repeated `resize()` calls (Story 3.18's fullscreen transition is the realistic trigger). Invisible to the suite because jsdom's `devicePixelRatio` is 1, so `× 1` is idempotent. Found independently by all three layers. [apps/web/lib/gridRenderer.ts:127-133]
- [x] [Review][Patch] **No renderer test draws more than one fill group — the `beginPath`/`fill`/`fillStyle` count assertions cannot fail** — every LUT/grid pair in `gridRenderer.test.ts` resolves to at most one group (`lut([0,5,5],[0,0,0])` maps both refs to token 5; `lut([0,3],[0,0])` is one organism; the rest are all-empty). So `expect(beginPath).toHaveLength(1)` cannot distinguish "one per group" from "once per draw": hoisting `beginPath()`/`fillStyle`/`fill()` **out** of the group loop — the exact O(n²) "last organism's colour wins" defect the source comment warns about — leaves the suite green. Likewise "two organisms sharing a colorToken yield one `fillStyle` write" asserts only the negative half of Decision B.2; nothing asserts two *different* tokens produce two writes. Three ticked subtasks (Task 5 line 160, Task 6 lines 183 and 185) claim coverage that does not exist. One fixture — `lut([0,5,9],[0,0,0])` with occupants `[1,2]` — closes all three. [apps/web/lib/gridRenderer.test.ts:132-210]
- [x] [Review][Patch] **`gridLayout.test.ts` never binds on the height axis — `Math.min`'s second term is untested** — every case is width-bound or an exact aspect tie (1000×600, 500×300, 2000×1200, 150×90, 400×240 are all 5:3 ties; 500×6000 and 997×601 are width-bound; the sweep uses `{width, height: width}` with `cols > rows`). The fast-check property asserts `cellSize * cols <= width || cellSize === 1` — the height axis is **not in the property at all**, and the `|| cellSize === 1` clause makes it vacuous in the overflow case it describes. Deleting `canvas.height / rows` from the `Math.min` leaves every test in the file green, while a 4000×100 canvas at 100×60 would then draw 2400px of grid into 100px of canvas. This is the precise failure the source comment claims to prevent. [apps/web/lib/gridLayout.test.ts:6-103, apps/web/lib/gridLayout.ts:33-34]
- [x] [Review][Patch] **`toRenderableGrid` silently truncates out-of-range cell values, contradicting its own "validates eagerly" doc comment** — it validates raggedness only. `occupant[i] = line[col]` into a `Uint8Array` coerces silently: `256 → 0` (the cell vanishes as empty), `-1 → 255` (a phantom organism), `1.9 → 1`, `NaN → 0`. A mis-migrated or hand-edited `gridState` therefore produces exactly the "plausible-looking but wrong dish" the comment says it exists to prevent, and `groupByColourState`'s out-of-range guard cannot help because the value has already wrapped into range. No test covers a value above 255 or below 0. [apps/web/lib/renderableGrid.ts:40]
- [x] [Review][Patch] **The groupId assertion is an algebraic tautology** — the implementation constructs `ageShade` as `groupId - lut.tokenIndex[ref] * 8`, so `expect(group.groupId).toBe(group.tokenIndex * 8 + group.ageShade)` reduces to `groupId === groupId`. True for every possible return value of `fillGroupOf`, including a completely broken one. Assert against an independently-derived expectation instead. [apps/web/lib/colourStateGroups.test.ts:72]
- [x] [Review][Patch] **Right and bottom grid-line bars are clipped off-canvas whenever the grid fits exactly** — vertical bars are drawn for `col = 0..cols`, so the final bar sits at `originX + drawWidth`, one pixel *past* the grid rectangle — directly contradicting the doc comment above it ("confined to the grid rectangle"). When the leftover space is 0, `originX === 0` and that bar lands at `x === canvas.width`, which `fillRect` silently clips to nothing. The grid then renders with a left and top border but no right or bottom border. The existing test passes because it counts `fillRect` *calls*, not painted pixels. [apps/web/lib/gridRenderer.ts:207-216]
- [x] [Review][Patch] **`resize()` blanks the canvas and then skips repainting it** — assigning `canvas.width`/`canvas.height` resets the bitmap even when the value is unchanged, and `resize()` always calls `applyDevicePixelSizing()` before taking the skip-repaint branch. Under `alpha: false` the cleared bitmap composites as opaque black, so a dimension-changing resize leaves a black rectangle where the dish should be until the caller's next `drawFull`. Same on the nothing-drawn-yet path. Guard the assignment on an actual value change. [apps/web/lib/gridRenderer.ts:272-284]
- [x] [Review][Patch] **Renderer tests hand-roll grid literals instead of building from `@gol/test-utils` through `toRenderableGrid`** — Task 6's final subtask is explicit: "Do not hand-roll grid literals — that is what AR-5 exists for." Both files define local builders over inline `Uint8Array`/`Uint16Array` literals. Consequence: `toRenderableGrid` — the Task 1 adapter — is imported by no file except its own test, so the dense→typed seam is never exercised alongside the renderer. [apps/web/lib/gridRenderer.test.ts:28-40, apps/web/lib/colourStateGroups.test.ts:12-19]
- [x] [Review][Patch] **Two unnecessary casts sit directly beneath a comment asserting the file carries none** — the `Canvas2D` alias comment says it exists so the double satisfies it "with no cast anywhere except the one spy-return-type coercion the test file names." Both casts are also redundant: `getContext('2d')` returns `CanvasRenderingContext2D | null`, which is assignable to `Pick<CanvasRenderingContext2D, …> | null` without assertion. Deleting them makes the comment true and loses nothing. [apps/web/lib/gridRenderer.ts:106, apps/web/lib/gridRenderer.ts:192]
- [x] [Review][Patch] **`devicePixelRatio` of 0 or NaN passes the `??` guard** — `??` catches only `null`/`undefined`. A `devicePixelRatio` of 0 (stubbed, or a headless/virtualised display) collapses the backing store to 0×0: negative origins, nothing renders, no error. Guard with `Number.isFinite(raw) && raw > 0`. [apps/web/lib/gridRenderer.ts:128]
- [x] [Review][Patch] **`groupByColourState` loops `occupant.length` rather than `width * height`, and never checks `age.length`** — as a public function it is reachable without the renderer's `assertGridMatchesSize`. An `occupant` longer than `width * height` emits phantom cells at `row >= height`, painted outside the grid rectangle; an `age` shorter than `occupant` yields `undefined → NaN`, which clamps to shade 0 and paints those cells at the newborn colour. Bound the loop by the declared dimensions and assert the two array lengths agree. [apps/web/lib/colourStateGroups.ts:35, apps/web/lib/colourStateGroups.ts:48]
- [x] [Review][Patch] **`computeGridLayout` produces `NaN` on non-finite canvas dimensions, and its zero-dimension guard is never exercised** — `Math.max(1, Math.floor(NaN))` is `NaN`, which propagates to every rect; the existing "0×0" test uses a 0×0 *canvas* with `cols:100/rows:60`, so the `cols > 0 && rows > 0` guard itself is never hit. Add the finite check and a genuine `cols === 0` case. [apps/web/lib/gridLayout.ts:33-40, apps/web/lib/gridLayout.test.ts:78]
- [x] [Review][Patch] **Three assertions are too weak to fail** — `expect(groups.length).toBeLessThanOrEqual(160)` where the fixture produces *exactly* 160 (passes for an implementation returning `[]`; should be `toBe(160)`); `groups[0].cells.sort()` uses the default lexicographic comparator, correct for `[0,1]` but wrong for any group containing a cell index ≥ 10; and the resize test asserts only `calls.length > callsAfterFirstDraw`, proving "did something" rather than the promised "recomputes the layout" — it never checks the new `cellSize` reaches a single rect. [apps/web/lib/colourStateGroups.test.ts:35, apps/web/lib/colourStateGroups.test.ts:88, apps/web/lib/gridRenderer.test.ts:682]
- [x] [Review][Patch] **The AC3 structural regex guards only `gridRenderer.ts`** — `colourStateGroups.ts`, `gridLayout.ts`, `refToFillGroup.ts`, and `renderableGrid.ts` are equally bound by the no-scheduling promise Epic 3 is being given, and none is checked. Widening the read to the module set costs one array. [apps/web/lib/gridRenderer.test.ts:472-475]
- [x] [Review][Patch] **The structural test's comment mis-states why its path resolves** — it credits `vitest.config.mts`'s `include`, which is a glob filter with no bearing on `process.cwd()`. The path works because Turbo runs `test` inside `apps/web`. The test itself is real and correct — only the stated justification is wrong, and a wrong WHY is worse than none under this project's comment convention. `import.meta.url` would make it robust and the comment true. [apps/web/lib/gridRenderer.test.ts:402-404]
- [x] [Review][Patch] **Untested boundaries that the code does handle** — `buildRefToFillGroup` is tested at roster 3 and at the 256-throw, never at the **255** boundary where `size === 256` and occupant values saturate; and `assertGridMatchesSize`'s `occupant.length !== cellCount` branch is never reached, because the only mismatch test trips the width/height check first. Both are cheap to add and both guard real corruption paths. [apps/web/lib/refToFillGroup.test.ts, apps/web/lib/gridRenderer.test.ts]
- [x] [Review][Patch] **`deferred-work.md`'s Story 1.7 `displayColorAt` entry is now resolved but still listed as open** — Task 2 closed it by resolving tokens at LUT-build time, and the reasoning holds (`buildRefToFillGroup` guarantees in-range `tokenIndex`; `ageShade` is derived as `groupId - tokenIndex*8 ∈ [0,7]`), so the clamp is never exercised on a corrupt index during a real render. The ledger entry should be struck with a pointer to this story. [docs/implementation-artifacts/deferred-work.md:53]
- [x] [Review][Defer] **The grid-line overlay cache's `drawImage` path has zero test coverage** [apps/web/lib/gridRenderer.ts:183-198] — deferred, disclosed in the Dev Agent Record
- [x] [Review][Defer] **`rebuildGridLineOverlay()` allocates a full-backing-store canvas on every `resize()`** [apps/web/lib/gridRenderer.ts:183-198] — deferred, no caller yet
- [x] [Review][Defer] **`renderStatic` retains `lastGrid`, pinning each tile's typed arrays for the surface's lifetime** [apps/web/lib/gridRenderer.ts:236] — deferred, Story 2.3 owns the divergence
- [x] [Review][Defer] **No `setPalette` — a roster change cannot update the LUT the renderer holds** [apps/web/lib/gridRenderer.ts:91] — deferred, no caller yet
- [x] [Review][Defer] **`groupByColourState`'s per-frame allocation profile** [apps/web/lib/colourStateGroups.ts:29-68] — deferred, pre-emptive
- [x] [Review][Defer] **`resize()` accepts NaN or fractional `cols`/`rows` unvalidated** [apps/web/lib/gridRenderer.ts:272] — deferred, no caller yet

_**Dismissed as noise (6):** the `gridLineOverlayLayout === this.layout` guard being unreachable-false (a harmless defensive invariant); `fillGroupOf` exported without a `ref` guard (every call site guards first **by design** — a guard there would put a branch in the per-cell loop the LUT exists to keep clean); Task 2's "slot 0 is never read" shipping as a prose comment (the spec's parenthetical was ambiguous and `colourStateGroups.test.ts`'s all-empty case covers the behaviour); `recordingContext2d`'s spy ignoring `contextId` (test double, no non-2D caller); `recordingContext2d.ts` importing `vitest` from `lib/` (deliberate, matches the `paletteCvd.ts` precedent, bundle verified unchanged); and a hand-built LUT with `tokenIndex > 19` (`buildRefToFillGroup` makes it unconstructible)._
