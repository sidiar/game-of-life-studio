...
11. **Gates.** `npm run ci:dev` green (typecheck, lint, format, `spec:check`, `boundary:check`,
    ...
    bench:check, Chromium e2e). axe passes with the status line visible on the Library, with the
    error line visible in the modal, and after a save-close. No new `--gol-*` token, no hex, no
...
- **No auto-dismiss on the Library outcome line (FD5).** The sentence stays until the next editor
  open. A 5–8 s dismissal is a `setTimeout` in `<OrganismLibrary>` plus fake-timer tests; deferred
  until the 4.17/4.22 flows show whether it reads as clutter.
...
Claude Opus 5 (1M context) — implement-next-story lane-epic-4 run, 2026-09-22.
