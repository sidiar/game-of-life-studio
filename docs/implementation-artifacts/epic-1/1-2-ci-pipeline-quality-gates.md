...
- [x] **Task 2: Vitest + coverage + RTL wiring** (AC: 4)
  ...
  - [x] `vitest-axe` proven in the same test — its `axe()` runner asserts `results.violations` on the rendered container (the `toHaveNoViolations` matcher is intentionally NOT wired; see `vitest.setup.ts` — same coverage, no Vitest-4 matcher-type conflict). NOTE: jsdom can't run axe's color-contrast rule (no canvas `getContext`) — it logs "Not implemented" and skips that rule; real color-contrast a11y is covered by the Playwright axe check (Task 3). `fast-check`@^4 installed at root for engine invariant tests (Epic 3).
...
- [x] [Review][Decision] AC1's e2e→axe gate ordering — **RESOLVED (Sidiar): accepted as implemented.** Running axe as a second `test()` within the same Playwright spec/run (not a separate CI stage) satisfies AC1's intent. No code change.
- [x] [Review][Patch] AR-46 colour-selector scope — **RESOLVED (Sidiar): extend now, not deferred to 1.7.** `HEX_COLOUR_SELECTOR` in `eslint.config.mjs:21` only matches quoted `#hex` string `Literal` nodes; widen it to also catch hex inside tagged template literals (the idiomatic Emotion `css\`...\`` pattern this project will use with MUI/Emotion) and `rgb()/rgba()/hsl()` functional colour notation. Named CSS colours (`"red"`) remain out of scope — too high a false-positive risk, stays with the `TODO(1.7)` palette-registry whitelist work. [eslint.config.mjs:21] — **Fixed:** replaced with `COLOUR_SELECTOR_GROUP` covering hex `Literal`, functional-colour `Literal`, and `TemplateElement` hex/functional matches. Verified: probe `rgb(255,0,0)` string + `` `color: #fff;` `` template literal both fire; `eslint .` clean on the real tree.
- [x] [Review][Patch] `.husky/pre-commit` can silently let a lint/format failure through — no `set -e` or `&&` chaining between `npx lint-staged` and `npm run typecheck`, so the hook's exit code is whatever typecheck returns, not lint-staged's. The committed blob is also mode 100644, not 100755. [.husky/pre-commit] — **Fixed:** added `set -e`; `chmod +x .husky/pre-commit` (now 100755).
- [x] [Review][Patch] AR-46 lint block has no test-file exemption, unlike the import-boundary block right below it — a test asserting a literal hex value will fail lint. [eslint.config.mjs:58-71] — **Fixed:** added the same `ignores: ['apps/web/**/*.test.{ts,tsx}', 'apps/web/**/*.spec.{ts,tsx}', 'apps/web/e2e/**']` as the import-boundary block.
- [x] [Review][Patch] `e2e` GitHub Actions job installs Playwright browsers unconditionally despite the cache step (`if: steps.playwright-cache.outputs.cache-hit != 'true'` is missing), and never restores the `.turbo` cache the `quality` job warms — `build:standalone` rebuilds cold every run. [.github/workflows/ci.yml:71-104] — **Fixed:** added the `if:` guard on the install step and a "Restore Turborepo cache" step to the `e2e` job (mirrors `quality`'s). YAML re-validated (Prettier-clean).
...
- [x] [Review][Defer] No hash/timestamp ties the bundle checked by `bundle:check` to the current commit's build — a stale `out/` from a prior run could theoretically be checked instead of a fresh one. [scripts/check-bundle-size.mjs] — deferred, speculative for the current single-job-chain local pipeline.
- [x] [Review][Defer] `packages/{domain,simulation,persistence,test-utils}/vitest.config.ts` are byte-identical (verified via md5) with no shared base config, so any future coverage-config change (e.g. the 3.7 gate flip) needs editing in four places. [packages/*/vitest.config.ts] — deferred, matches this repo's established per-package-owns-its-config convention; a shared base is a nice-to-have, not a defect.
- [x] [Review][Defer] GitHub Actions are pinned by major-version tag (`@v4`) rather than commit SHA — standard supply-chain hardening, but adds ongoing maintenance cost. [.github/workflows/ci.yml] — deferred, low priority for a single-maintainer repo with no remote yet.
...
- 2026-08-03: Code review (fresh context, Sonnet 5) — 3 parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) against the Story 1.2 diff. 2 decision-needed resolved by Sidiar (AR-46 selector scope extended now; AC1 e2e+axe combined-run accepted as implemented), 9 patches applied and verified (`npm run ci` exit 0, 8/8 e2e), 5 items deferred to `deferred-work.md`, 7 raised findings dismissed as false positives after verification against the actual repo state. See Review Findings above. Status → done.
