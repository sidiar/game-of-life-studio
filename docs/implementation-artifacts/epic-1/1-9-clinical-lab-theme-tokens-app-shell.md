...
- [Source: docs/implementation-artifacts/epic-1/1-8-gridrenderer-static-core.md#Dev Notes] — the injected-colour constructor parameter awaiting `--gol-bg-primary` / `--gol-border`, and the `npm run ci` / ticked-but-unshipped lessons
- [Source: docs/implementation-artifacts/epic-1/1-7-palette-token-registry-display-color-lut.md] — organism colours are **not** theme tokens (RFC-007 Decision 5); the two systems must not reference each other
