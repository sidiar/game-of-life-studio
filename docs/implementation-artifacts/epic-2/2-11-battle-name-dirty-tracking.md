...
**Forced decision 6 — enforcing the cap (AC1).** ⚠️ **Reversed after review, on Sidiar's call (2026-08-28): option (b), a clamp in the change handler, ON TOP OF the native attribute.** Originally implemented as (a), native `maxLength` only. Review established that the DOM attribute is a UA guarantee for ordinary typing alone — an IME commit, browser voice dictation, `document.execCommand('insertText')` and a password-manager/autofill write each fire an `input` event whose value is already over the cap, and nothing downstream clamped it, so the counter could read "104 / 100" with no error state over a name `BattleSchema.name` rejects at Story 2.13's save. `BattleNameField` now calls `onChange(event.target.value.slice(0, maxLength))`. Both mechanisms are kept: the attribute stops typing at the UA (cheapest, and preserves the caret), the clamp catches everything the attribute does not cover. Pinned by three tests in `BattleNameField.test.tsx` — the original controlled-round-trip typing test, plus two `fireEvent.change` tests (`user.type` honours the attribute and so cannot reach the uncovered paths) covering the handler contract and the counter's denominator. Accepted cost, flagged when the option was offered: an over-long paste is silently truncated rather than shown and refused.
