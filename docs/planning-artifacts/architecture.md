...
- **H.4 — `BattleSummary` is defined and carries the placed `organismIds`.** `{ id, name, gridSize, organismIds, updatedAt }` — everything but the heavy `gridState`. The Decision 8 usage index builds from `list()` with zero grid deserialization; thumbnails stay on-demand from `load()` (M4).
