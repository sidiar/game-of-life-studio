...
- **H.4 — `BattleSummary` is defined and carries the placed `organismIds`.** `{ id, name, gridSize, organismIds, updatedAt }` — everything but the heavy `gridState`. The Decision 8 usage index builds from `list()` with zero grid deserialization; thumbnails stay on-demand from `load()` (M4). *(2026-08-08, Story 1.10: confirmed to exclude `createdAt` — FR-7.3's "Date created / last modified" is ONE field, and `updatedAt` already is it, equal to `createdAt` until a battle is first edited.)*
...
3. **Battle grid shape — dense at rest, sparse on the wire.** Persisted: dense `gridState: number[][]` (RFC-001; `0` = empty, `v` = index+1 into `organismIds`). Runtime: typed-array `Grid` (RFC-004). Wire/export: sparse `cells[]` (RFC-006). **`@gol/simulation` converts dense↔typed at the runtime boundary** (`gridFromDense` / `gridToDense`, Story 3.3) — not the battle repository, which returns and stores the dense form as-is: the typed `Grid` is RFC-004's and lives in `@gol/simulation`, so a repository-owned conversion would need a `@gol/persistence → @gol/simulation` edge, stopping persistence being a leaf over `@gol/domain` and reversing the direction the DI seam protects (AR-2/27). The `WorkspaceSerializer` converts dense↔sparse at export/import. *(Product choice; dense at rest is heavier in localStorage than sparse — within NFR-7.2, covered by the FR-8.2 meter.)*
...
- **M11 — RFC-004 Part 1's generic signatures are amended to what Story 3.1 built; the §1.5 schema helper is retired (Story 3.1, 2026-09-08).** Two defects found while implementing the engine, neither a cross-document conflict — both are RFC-004 disagreeing with itself. **(a) The `P` collision.** §1.1 declared `Condition<P = unknown>` where `P` was the *pattern* type, then never supplied it (`Rule.conditions` was a bare `Condition[]`; every §1.4 signature took a bare `Condition`), while §1.4 reused `P` for the *payload*. One letter, two meanings, the pattern one inert. **Resolved by dropping the pattern parameter** — `pattern: unknown` already matches the `Predicate` contract §1.2 declares and keeps the generic `Condition` assignable *from* the concrete §2.4 one — and threading `Props` (the property-name key set) instead, which genuinely flows through `Rule.conditions`. The payload parameter is spelled `Payload`. **(b) `Selectors<S>` was unsafe by default.** `Record<string, Selector<S>>` is *total* over every string key with `noUncheckedIndexedAccess` off, so a dictionary missing an entry type-checked and the lookup returned a `Selector` the runtime did not have. `Props` is now **mandatory on `Selectors`** — a default would restore the hole for anyone who says nothing — while `Condition`/`Rule`/`RuleSet` keep their `string` default, where it widens a property name rather than faking a lookup (and where **AC5 requires a bare `RuleSet<SurvivalPayload>`** to stay legal). **(c) §1.5's `makeRuleSchema` helper is retired**, not deferred: its only intended caller already exists and bypasses it — Story 1.3 authored the concrete `ConditionSchema`/`SurvivalRuleSchema` directly in `@gol/domain` from §2.4 — and building it would add a `zod` dependency to `packages/simulation`, pulling a parsing library into the engine package that `project-context.md` keeps at the boundaries. Construction-is-parsing still holds; validation stays at the persistence boundary (§2.4) and the engine exposes **no schema surface**. **Owner:** RFC-004 (§1.1/§1.3/§1.4 signatures, §1.5 marked stale, SOLID/DIP bullet). **No PRD impact** — these are internal type signatures. *(Observation recorded, not re-opened: §1.1's `NumericOperator | EqualityOperator` split names the same six operators, `'eq'` being an arm of both.)*
- **M12 — The rules engine fails closed on malformed input rather than throwing inside the 60 FPS loop (Story 3.1, 2026-09-08).** RFC-004 §1.2/§1.4's reference code cast (`v as number`) and indexed unguarded, on the rule that "inside the engine types are already proven — never re-parse per cell". That rule **stands and is unchanged** — there is no schema work, no Zod, no re-parse — but it proved too much: the casts are *erased at runtime*, and a `Selector<S>` returns `unknown` and may legitimately yield null/undefined, which JS then coerces (`null` → 0, so **`null > -1` was silently TRUE**; `undefined` → NaN, so every comparison was silently false). `range` destructured its pattern unchecked, so a non-iterable **threw**; a 1-element array silently never matched. `Operator` is a compile-time union, so a retired id — `ne`, valid until **Decision C** — made `operators[...]` undefined and **calling it threw**. Resolution: each predicate tests whether the comparison is *meaningful* before answering it, and both hot-path lookups are guarded, returning `false` (condition unsatisfied) rather than propagating a crash. Two traps are recorded because the obvious fix misses them: the operator dictionary needs a **null prototype** (a plain literal inherits `Object.prototype`, so `operators['toString']` resolves to a real function, passes an `undefined` check and gets *called*), and the selector guard must be **`Object.hasOwn`, not `typeof === 'function'`** (same inheritance, and `Object.prototype.hasOwnProperty` invoked with `this === undefined` **throws**). Cost measured, not asserted: **+0.019 ms/cycle** for the operator guards and **+0.091 ms/cycle** for `Object.hasOwn`, against the **16.7 ms NFR-1.1 frame budget** (~0.1% and ~0.5%). ⚠️ **Both figures were superseded on re-measurement — see the amendment below.** Failing closed is deliberate but silent, so **eager diagnostics belong at evaluator-compile time** — one pass per battle, not one per cell — i.e. RFC-004 §3.5's `contentHash`-keyed precompilation (Story 3.4), after which the per-cell guards become redundant and should be re-measured with Story 3.7's harness — **which happened, and moved them; see the amendment.** **Owner:** RFC-004 (§1.2 operator dictionary, §1.4 evaluation cascade, §3.5 compile-time sweep). **No PRD impact.**

  **AMENDED 2026-09-10 (Story 3.7), on Sidiar's explicit authorization — the M14/M15 precedent.** Two things changed: the recorded cost, and where the guards sit on the hot path.

  **The cost was ~15x understated.** Re-measured with Story 3.7's harness at the real NFR-1.1 baseline — 100x60, 20 organisms, 50 rules, each guard removed and restored in an interleaved A/B — the operator `typeof` guards cost **0 ± 0.3 ms/cycle** (below the harness's own run-to-run spread, confirming the +0.019 ms above), but `Object.hasOwn` cost **+1.4 ms/cycle**: ~10% of the cycle and 8% of the frame budget, against the **+0.091 ms** recorded here. The original figure was taken on a far smaller evaluation count; the real baseline makes ~300,000 `Object.hasOwn` calls per cycle (120,000 subjects x ~2.5 conditions each). Measurement, method and the full table: `docs/implementation-artifacts/performance-baseline-validation.md`.

  **The guards MOVED; none was removed, and no caller is less safe.** M12's own text anticipated this: the per-cell guards *"become redundant"* once RFC-004 §3.5's compile-time sweep exists. It does (`validateSurvivalRules`, Story 3.4), and its `isCellProperty` check **is** `Object.hasOwn(cellSelectors, property)` while its `OPERATORS.includes` check **is** the operator dictionary's totality test — the same two questions, asked once per condition per battle instead of once per condition per cell. Story 3.7 therefore compiles each condition down to a concrete `(cell) => boolean` at session time (`compileEvaluators.ts`), resolving selector and predicate once, so the compiled hot path performs **neither lookup and neither guard**. Measured effect on the assembled cycle: **12.2-14.4 ms → 5.7-6.1 ms**, ~2.3x, which is what took the gated NFR-1.1 frame from 18.7 ms (red) to 6.8 ms against a 16.667 ms budget.

  ⚠️ **`firstSatisfiedBy` and `operators` are UNCHANGED and keep every guard this resolution describes**, including the null prototype and the `Object.hasOwn`-not-`typeof` trap. They are parametric over an arbitrary subject (AR-16) and reachable by callers that never compile a session — `resolveCellAction` is one — and those callers have no sweep standing behind them. What changed is that the 60 FPS loop no longer routes through them. The safety property, stated so a later story cannot erode it by accident: **a caller may skip `compileSession` and still be guarded per cell, or go through `compileSession` and be guarded once before its first cycle — but never neither.** Story 4.15's draft organism is the second case.

  ❌ **Explicitly NOT authorized and still out of scope:** narrowing the candidate organism set per cell (not evaluating every organism at every cell). That is a semantics-bearing change to M10 / Decision C behaviour and needs its own story and its own goldens.
- **M13 — The domain↔engine assignability pin is forward at the container level and bidirectional at the payload level; AC4's flat "both directions" is amended (Story 3.2, 2026-09-09).** RFC-004 §2.4 claims persisted data feeds the engine **without any mapping layer**, and the only thing that can prove that claim is `tsc` — hence the pin in `packages/simulation/src/domainRuleSetCompatibility.test.ts`. Story 3.2's AC4 escalated §2.4's claim to "assignable **both directions**" flat, across every level. That is simultaneously unachievable and unnecessary, and the split is structural rather than a shortfall in the implementation. **(a) Container level (`RuleSet` / `Rule` / `Condition`) — forward only, architecturally EXCLUDED in reverse.** Two blockers, both AR-16 properties of `src/engine/`: engine `RuleSet` is a `readonly` array and TypeScript never assigns a readonly ARRAY into a mutable-typed reference (TS4104) regardless of element shape; and the engine's `Condition<Props>` is one flat `pattern: unknown` shape — a strict SUPERTYPE of `@gol/domain`'s discriminated union — which can never reverse-assign into the narrower union without a cast. Narrowing the engine's `Condition` to permit it would re-introduce exactly the GoL-awareness AR-16 exists to keep out of `src/engine/`, so this is **closed, not deferred — there is no owner and no residual work**. §2.4 only ever claimed the forward direction, so this half is the RFC's own claim, restored. **(b) Payload level (`SurvivalPayload` / `Action`) — BOTH directions, and this is the half that is load-bearing.** `readonly` PROPERTY modifiers (unlike readonly arrays) are ignored for assignability and `Action` is a plain string union, so nothing structural blocks the reverse. Story 3.2's FD1 duplicates `Action`/`SurvivalPayload` in `packages/simulation` rather than importing `@gol/domain`'s, **on the stated grounds that drift becomes a build failure here** — and a forward-only assignment is **covariant**, so it cannot deliver that: widening the ENGINE's copy still compiles. Proven by mutation during review — adding a fourth `Action` member left `tsc` at exit 0 before the reverse pins existed, and fails with them. **The generalizable rule, which is why this is an M-entry and not a story note: any story that duplicates a type across the domain/engine seam must pin it BIDIRECTIONALLY at the level it duplicated.** Forward-only detects drift in the persisted copy and is blind to widening in the engine's. **Owner:** RFC-004 (§2.2 payload types, §2.4 the no-mapping-layer claim) + Story 3.2's AC4, amended to "forward at container level, bidirectional at payload level". **No PRD impact** — these are internal type signatures.
- **M14 — An `OrganismRef` is the battle roster index **+ 1**, with `0` reserved for "empty"; RFC-004 §2.1's wording and §3.2's `resolveConflict` snippet are corrected to match the encoding every shipped layer already uses (Story 3.3, 2026-09-09).** The project carried two incompatible definitions of the same number, and both were written down. RFC-004 §2.1 called an `OrganismRef` "the index into the dense `organisms` array", and §3.2's `resolveConflict` read `deps.organisms[r].dominance` accordingly; `packages/simulation/src/gol/cellSubject.ts` repeated that reading. Against them, the persisted dense `gridState` (RFC-001 — `0` = empty, `v` = index+1, with `BattleSchema` validating `v <= organismIds.length`), `apps/web/lib/canvas/refToFillGroup.ts` (which allocates `roster length + 1`), and all of Epic 1 and Epic 2 mean **`ref = index + 1`**. The two readings cannot both hold: under the bare-index reading `organisms[0]` is a real organism and collides with occupant `0` = empty. **The shipped convention wins** — it is what is on disk in every saved battle, and reversing it would mean migrating persisted data to fix documentation. **The consequence, stated once so Stories 3.5/3.6 need not re-derive it: a roster lookup off a ref is `organisms[ref - 1]`, never `organisms[ref]`.** This is precisely where the defect would otherwise have surfaced — §3.2's snippet is the Dominance comparison, so an off-by-one there reads the *wrong organism's* dominance and produces plausible battles with no failing test (Story 3.6). Story 3.3 settled the encoding in code (`packages/simulation/src/grid/grid.ts` carries the single written statement) and corrected `cellSubject.ts`; this entry is the authority-doc half, minted on Sidiar's explicit authorization after the story deliberately declined to mint it unilaterally. **Owner:** RFC-004 (§2.1 `OrganismRef` definition + type comment, §3.2 `resolveConflict`, §3.4 the `occupant` field comment). **No PRD impact** — this is a runtime representation, confined to the simulation session by [Decision E](#decision-e--stable-organism-ids-at-rest-numeric-refs-are-runtime-only). *(`scripts/check-spec-ids.mjs`'s bounded `M1`–`M13` range was widened to `M14` in the same change — the widening its own comment prescribes.)*
- **M15 — A compiled organism evaluator is a **phase-partitioned pair** (`resolvesToDeath` → `boolean`, `resolveBirthSurvival` → `Action | null`), not one `resolveAction`; RFC-004 §3.1's `SimulationDeps` signature is amended to match §2.3/§3.5 (Story 3.4, 2026-09-10).** RFC-004 contradicted itself. §3.1 declared a single injected `resolveAction: (organism: OrganismRef, cell: CellSubject) => Action | null` — one phase-agnostic function — while §2.3, §3.5 and AR-18 required **two** evaluators per organism, partitioned by action type at compile time. Both sentences were in the same RFC and cannot both be the whole truth. **The pair wins, and not as a close call:** phase partitioning is what makes [M10](#minor-spec-resolutions)'s Die-vs-Survive precedence structural rather than order-dependent, and M10 is a cross-cutting Decision, which outranks an RFC line by the authority order. The PRD decision log confirms this is a missed edit rather than a live disagreement: the H-5 change already rewrote §2.3 and §3.5 onto `resolvesToDeath` / `resolveBirthSurvival`, explicitly retiring the *"order-authoritative `resolveAction == 'die'` death phase"* — §3.1's interface block was simply not updated in that pass. **The consequence, stated once so Stories 3.5/3.6/3.8 and 4.15 need not re-derive it: a strategy reads `evaluatorsByRef[ref]` and calls the two members; there is no `deps.resolveAction`.** The compiled table is ref-indexed with `length = roster length + 1` and slot `0` an explicit `null` ([M14](#minor-spec-resolutions) — slot 0 is the reserved EMPTY value and belongs to no organism). ⚠️ `resolveBirthSurvival` returning `null` means **no rule matched**, never `'die'`: a living cell matching nothing dies at cycle end by implicit death (M10) and remains a Phase-2 neighbour until then, which is what preserves Conway's simultaneous-generation semantics — Conway's Classic has no `die` rule at all, so collapsing `null` into `'die'` breaks it while looking equivalent. Story 3.4 built the pair in `packages/simulation/src/session/compileEvaluators.ts` and recorded the tension without resolving it; this entry is the authority-doc half, minted on Sidiar's explicit authorization after the story deliberately declined to mint it unilaterally (the M14 precedent). **Owner:** RFC-004 (§3.1 `SimulationDeps`, §3.5 DIP prose, the Design-patterns table row). **No PRD impact** — the PRD never specified the evaluator's shape, and its decision log already records the phase-partitioned form. *(`scripts/check-spec-ids.mjs`'s bounded `M1`–`M14` range was widened to `M15` in the same change — the widening its own comment prescribes.)*
