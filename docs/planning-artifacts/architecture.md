...
- **H.4 — `BattleSummary` is defined and carries the placed `organismIds`.** `{ id, name, gridSize, organismIds, updatedAt }` — everything but the heavy `gridState`. The Decision 8 usage index builds from `list()` with zero grid deserialization; thumbnails stay on-demand from `load()` (M4). *(2026-08-07, Story 1.10: `createdAt` briefly added, optional, to back a Gallery-tile disclosure panel showing both dates. Reverted 2026-08-08 in the same story once the tile settled on FR-7.3's "Date created / last modified" as ONE field — `updatedAt` alone, which already equals `createdAt` until a battle is first edited — leaving the field with no consumer.)*
