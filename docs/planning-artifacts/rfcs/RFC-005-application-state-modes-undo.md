...
- **The index is buildable from `list()` by contract (arch Decision H.4):** `BattleSummary` (RFC-001) carries the battle's `organismIds` — which at rest is **exactly the placed set** (Decision H.1) — alongside id/name/gridSize/updatedAt (and, since 2026-08-07's Story 1.10, an optional `createdAt`). `buildUsageIndex` is a flat map over summaries; **no grid is ever deserialized** for the Library/Editor checks (no `listFull()` fallback).
